...
def print_prefix(output=sys.stdout, debug_mode=False, error_mode=False, prefix=None, indent=True):
...
def logline_proxy(
    log_file, *args, spaces_between: bool = True, end_line: bool = True, is_debug: bool = False, is_error: bool = False
):
...
def gen_proxy(log_file, is_debug=False, is_error=False):
    ...
    return lambda *args, spaces_between=True, end_line=True: logline_proxy(
        log_file, *args, spaces_between=spaces_between, end_line=end_line, is_debug=is_debug, is_error=is_error
    )
