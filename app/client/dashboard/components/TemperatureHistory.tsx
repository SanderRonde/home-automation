...
export const TemperatureHistory = (): JSX.Element => {
	...
	const loadTemperatureData = async () => {
		try {
			...
			const devicesResponse = await apiGet('device', '/list-with-values', {});
			...
			if (sensorsData.temperatureControllers) {
				for (const controllerId of sensorsData.temperatureControllers) {
					...
					const historyResponse = await apiGet('temperature', '/temperature/:deviceId/:timeframe', {
			...
			if (sensorsData.deviceSensors) {
				for (const deviceSensor of sensorsData.deviceSensors as DeviceSensor[]) {
					...
					const currentTemp =
						device?.clusters?.find(
							(c: { name: string; temperature?: number }) => c.name === 'temperatureMeasurement'
						)?.temperature ?? null;
		...
		<Box sx={{ p: { xs: 2, sm: 3 } }}>
			<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
				...
				<Typography variant="body2" color="text.secondary">
					View temperature readings from all sensors with historical trends and current values.
				</Typography>
				...
				{sensors.length === 0 ? (
					<Card>
						...
						>
							<DeviceThermostatIcon sx={{ fontSize: 64, color: 'text.secondary', opacity: 0.5 }} />
												...
												<CardContent sx={{ flexGrow: 1 }}>
													...
													>
														<Box sx={{ flexGrow: 1 }}>
															...
															<Chip
																label={sensor.type === 'controller' ? 'Controller' : 'Device'}
																size="small"
																variant="outlined"
																sx={{ fontSize: '0.7rem' }}
															/>
													...
													{sensor.history.length > 0 ? (
														<Box sx={{ mt: 2, height: 200 }}>
															<Line
																data={formatChartData(sensor.history)}
																options={{
																	responsive: true,
