					...
					<Box>
						...
						{triggerType === SceneTriggerType.OCCUPANCY && (
							<Autocomplete
								...
								value={
									occupancyDevices.find((d) => d.uniqueId === triggerDeviceId) ??
									null
								}
						...
						{triggerType === SceneTriggerType.BUTTON_PRESS && (
							<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
								<Autocomplete
									...
									getOptionLabel={(option) => option.name}
									value={
										buttonDevices.find((d) => d.uniqueId === triggerDeviceId) ??
										null
									}
									onChange={(_e, newValue) => {
										setTriggerDeviceId(newValue?.uniqueId ?? '');
						...
						{triggerType === SceneTriggerType.HOST_ARRIVAL && (
							<Autocomplete
								options={props.hosts}
								getOptionLabel={(option) => option.name}
								value={props.hosts.find((h) => h.name === triggerHostId) ?? null}
								onChange={(_e, newValue) => {
						...
						{triggerType === SceneTriggerType.WEBHOOK && (
							<Autocomplete<Webhook>
								options={webhooks}
								getOptionLabel={(option) => option.name}
								value={webhooks.find((w) => w.name === triggerWebhookName) ?? null}
								onChange={(_e, newValue) => {
									...
									{conditionType === SceneConditionType.DEVICE_ON && (
										<>
											<Autocomplete
												...
												onChange={(_e, newValue) => {
													setConditionDeviceId(newValue?.uniqueId ?? '');
												}}
												renderInput={(params) => (
													<TextField
														{...params}
											...
											<FormControlLabel
												control={
													<Switch
														...
														onChange={(e) =>
															setConditionShouldBeOn(e.target.checked)
														}
													/>
												}
												label={
