...
export namespace RemoteControl {
	...
	export namespace GetSetListener {
		...
		export function addListener(command: Commands, listener: (command: Commands, logObj: any) => void, once: boolean = false) {
			const index = _lastIndex++;
			_listeners.set(index, {
				key: command['action'], listener, once, any: false
			});
			return index;
		}
		
		export function listenAny(listener: (command: Commands, logObj: any) => void, once: boolean = false) {
			const index = _lastIndex++;
			_listeners.set(index, {
				any: true, listener, once, key: ''
			});
			return index;
		}
		...
		export class Bot extends BotState.Base {
			...
			static readonly matches = Bot.createMatchMaker(({
				...
				mm(/play( (music|netflix|youtube|vlc|movie))?/, async ({ logObj }) => {
					new External.Handler(attachMessage(logObj, 'Playing')).play();
					return 'Playing';
				});
				mm(/pause( (music|netflix|youtube|vlc|movie))?/, async ({ logObj }) => {
					new External.Handler(attachMessage(logObj, 'Pausing')).play();
					return 'Pausing';
				});
				mm(/playpause( (music|netflix|youtube|vlc|movie))?/, async ({ logObj }) => {
					new External.Handler(attachMessage(logObj, 'Playpausing')).play();
					return 'Playpausing';
				});
				mm(/close( (music|netflix|youtube|vlc|movie))?/, async ({ logObj }) => {
					new External.Handler(attachMessage(logObj, 'Closing')).play();
					return 'Closing';
				});

				mm(/(?:increase|up) volume( by (\d+))?/, async ({ logObj, match }) => {
					new External.Handler(attachMessage(logObj, 'Increasing Volume')).volumeUp(match[1] ? 
						parseInt(match[1], 10) : undefined);
					return 'Increasing Volume';
				});
				mm(/(?:decrease|reduce|down) volume( by (\d+))?/, async ({ logObj, match }) => {
					new External.Handler(attachMessage(logObj, 'Decreasing Volume')).volumeUp(match[1] ? 
						parseInt(match[1], 10) : undefined);
					return 'Decreasing Volume';
				});

				mm(/set volume to (\d+)/, async ({ logObj, match }) => {
					new External.Handler(attachMessage(logObj, 'Setting Volume')).setVolume(parseInt(match[1], 10));
					return 'Setting Volume';
				});
				
				mm('/help_remote_control', /what commands are there for remote-control/, async () => {
	...
	export namespace API {
		export class Handler {
			...
			@errorHandle
			@authAll
			public static pause(res: ResponseLike, { }: {
				auth?: string;
			}) {
			...
			@errorHandle
			@authAll
			public static playpause(res: ResponseLike, { }: {
				auth?: string;
			}) {
			...
			@errorHandle
			@authAll
			public static close(res: ResponseLike, { }: {
				auth?: string;
			}) {
			...
			@errorHandle
			@authAll
			public static volumeUp(res: ResponseLike, { amount = 10 }: {
				auth?: string;
				amount?: number;
			}) {
				const msg = attachMessage(res, `Command: "volumeUp", amount: "${amount}"`);
				GetSetListener.update({
					action: 'volumeUp',
					amount: amount
				}, attachMessage(msg, "Updates"));
			...
			@errorHandle
			@authAll
			public static volumeDown(res: ResponseLike, { amount = 10 }: {
				auth?: string;
				amount?: number;
			}) {
				const msg = attachMessage(res, `Command: "volumeDown", amount: "${amount}"`);
				GetSetListener.update({
					action: 'volumeDown',
					amount: amount
				}, attachMessage(msg, "Updates"));
			...
			@authAll
			public static setVolume(res: ResponseLike, { amount }: {
				auth?: string;
				amount: number;
			}) {
				const msg = attachMessage(res, `Command: "setVolume", amount: "${amount}"`);
				GetSetListener.update({
					action: 'setVolume',
					amount
				}, attachMessage(msg, "Updates"));
	...
	export namespace Routing {
		export async function init({ 
			...
			app.post('/remote-control/play', async (req, res) => {
				API.Handler.play(res, {...req.params, ...req.body, cookies: req.cookies});
			});
			app.post('/remote-control/pause', async (req, res) => {
				API.Handler.pause(res, {...req.params, ...req.body, cookies: req.cookies});
			});
			app.post('/remote-control/playpause', async (req, res) => {
				API.Handler.playpause(res, {...req.params, ...req.body, cookies: req.cookies});
			});
			app.post('/remote-control/close', async (req, res) => {
				API.Handler.close(res, {...req.params, ...req.body, cookies: req.cookies});
			});

			app.post('/remote-control/volumeup/:amount?', async (req, res) => {
				API.Handler.volumeUp(res, {...req.params, ...req.body, cookies: req.cookies});
			});
			app.post('/remote-control/volumedown/:amount?', async (req, res) => {
				API.Handler.volumeDown(res, {...req.params, ...req.body, cookies: req.cookies});
			});

			app.post('/remote-control/setvolume/:amount', async (req, res) => {
				API.Handler.setVolume(res, {...req.params, ...req.body, cookies: req.cookies});
			});

			ws.all('/remote-control/listen', async ({ send, onDead, addListener }) => {
				...
				addListener((message) => {
					...
					if (Auth.Secret.authenticate(message)) {
						...
						const listener = GetSetListener.listenAny((command, logObj) => {
