...
export const HomeWizard = new (class HomeWizard extends ModuleMeta {
	...
	private async _fetchMeasurement(ip: string): Promise<MeasurementResponse | null> {
		try {
	...
	private async _poll(): Promise<void> {
		...
		const measurement = await this._fetchMeasurement(ip);
		...
		if (measurement) {
			...
			if (!device) {
				// Fetch device info
				try {
					const infoResponse = await fetch(`http://${ip}/api`, {
						method: 'GET',
					});
